...
class SCFP_FormEntries extends Agp_Module {
    ...
    public function dataCsv(){
        ...
        foreach( $data as $datakey => $datavalue ){
            if( !empty($datavalue['field_type']) && $datavalue['field_type'] != 'captcha' && !empty($datavalue['visibility']) && !empty($datavalue['exportCSV']) ){
               $header[] = strip_tags( html_entity_decode( $datavalue['name'] ) );
            }
        }
            ...
            foreach( $data as $datakey => $datavalue ){
                if( !empty($datavalue['field_type']) && $datavalue['field_type'] != 'captcha' && !empty($datavalue['visibility']) && !empty($datavalue['exportCSV']) ){
                    if( $datavalue['field_type'] == 'checkbox' ){
                        $row =  strip_tags( html_entity_decode( SCFP()->getChoicesView(get_post_meta( $post_ID, 'scfp_'.$datakey, true ), $datakey) ) ); 
    ...
    public function entrieColumns( $columns ){
        ...
        foreach( $fields as $key => $value ):
            if (!empty( $value['visibility'] ) && $value['field_type'] !== 'captcha' && $value['field_type'] !== 'textarea') :
                $results[$key] = strip_tags( html_entity_decode( $value['name'] ) );    
            endif;
        endforeach;
                ...
                foreach( $fields as $key => $value ):
                    if ($key == $column && !empty( $value['visibility'] ) && $value['field_type'] !== 'captcha' && $value['field_type'] !== 'textarea') :
                        if ($value['field_type'] == 'checkbox') {
                            echo strip_tags( html_entity_decode( SCFP()->getChoicesView(get_post_meta( $post->ID, "scfp_{$key}", true ), $key) ) ); 
                        } else {
    ...
    public function getReplyButton ($postId, $title = '', $atts = array()) {
        ...
        $nameField = !empty($settings['user_name']) ? $settings['user_name'] : '';
        $subjectField = !empty($settings['reply_subject_field']) ? $settings['reply_subject_field'] : '';
        $messageField = !empty($settings['reply_message_field']) ? $settings['reply_message_field'] : '';
        ...
        if (!empty($mail)) {
            ...
            if ($message) {
                ...
                $params[] = 'body=%0D%0A' .$name. '> '. str_replace('<br />', '%0D%0A', $message);    
            }
