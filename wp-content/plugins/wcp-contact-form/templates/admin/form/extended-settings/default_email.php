...
<div class="scfp-field-extended-settings-row">
    ...
    <input class="widefat" type="text" value="<?php echo isset($data['default']) && !is_array($data['default']) ? $data['default'] : '{$loggedin_user_email}' ;?>" id="<?php echo "{$id}_{$row}_default";?>" name="<?php echo "{$id}[field_settings][{$row}][default]";?>" />    
