...
if (!empty($key)) :
    ...
    if (isset($value) && is_numeric($value)) {
        if (!empty($min) && is_numeric($min) && $value < $min || !empty($max) && is_numeric($max) && $value > $max) {
            $value = '';
        }    
    }
?>
    <div class="scfp-form-row<?php echo !empty($css_class) ? ' '.$css_class : '';?>">
        ...
        <input<?php if (!empty($placeholder)):?> placeholder="<?php echo $placeholder;?>"<?php endif;?> <?php echo $atts;?><?php if (!empty($min) && is_numeric($min)):?> min="<?php echo $min;?>"<?php endif;?><?php if (!empty($max) && is_numeric($max)):?> max="<?php echo $max;?>"<?php endif;?> class="scfp-form-field" type="number" id="scfp-<?php echo $key; ?>" name="scfp-<?php echo $key; ?>" <?php if ( !empty( $required ) && !empty($formSettings['html5_enabled']) ) : ?> required <?php endif;?> value="<?php echo $value; ?>">
